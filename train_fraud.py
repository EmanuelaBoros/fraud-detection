...
from transformers import (get_linear_schedule_with_warmup,
                          ...
                          AutoTokenizer, AutoConfig, LayoutLMv2Processor)
...
def evaluation(model, epoch, test_df, loader):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(loader),
            ...
            fin_probabilities.extend(
                torch.max(
                    nn.Sigmoid()(logits),
                    dim=1).values.cpu().detach().numpy().tolist())
    ...
    try:
        fin_outputs = np.where(
            fin_outputs < np.mean(fin_outputs), 0, fin_outputs)
        fin_outputs = np.where(
            fin_outputs >= np.mean(fin_outputs), 1, fin_outputs)
        print(fin_outputs[:10])
    ...
    with open(os.path.join(output_dir, 'test.results.' + str(epoch) + '.txt'), 'w', encoding='utf-8') as f:
        for y_true, y_predicted, y_proba, image_path, text_path in zip(
                fin_targets, fin_outputs, fin_probabilities, test_df.image, test_df.text):
            f.write(image_path +
                    '\t' +
                    text_path +
                    '\t' +
...
if __name__ == '__main__':
    ...
    train_df['id'] = train_df['text'].apply(
        lambda x: int(x[x.index('txt/') + 4:-4]))
    test_df['id'] = test_df['text'].apply(
        lambda x: int(x[x.index('txt/') + 4:-4]))
    valid_df['id'] = valid_df['text'].apply(
        lambda x: int(x[x.index('txt/') + 4:-4]))
    ...
    entities_df['produits : quantité, prix unitaire, somme prix'] = entities_df['produits : quantité, prix unitaire, somme prix'].apply(
        lambda x: ' '.join([y.lower().replace('_', ' ') + ' '.join([str(price) for price in list(z)]) for y, z in zip(eval(x).keys(), eval(x).values())]))

    entities_df['nombre articles'].fillna(
        (entities_df['nombre articles'].mean()), inplace=True)

    entities_df['paiement intermediaire : montant, moyen paiement, monnaie rendue'] = entities_df['paiement intermediaire : montant, moyen paiement, monnaie rendue'].apply(
        lambda x: ' '.join([' '.join([str(price) for price in list(z)]) for y, z in zip(eval(x).keys(), eval(x).values())]))

    entities_df['montant paye'].fillna(
        (entities_df['montant paye'].mean()), inplace=True)
    ...
    for column in [
        ...
        'paiement intermediaire : montant, moyen paiement, monnaie rendue',
            'montant paye']:
        entities_df[column] = entities_df[column].apply(
    ...
    if args.do_train:
        ...
        text_transformer = TfidfVectorizer(ngram_range=(
        ...
        config_image = AutoConfig.from_pretrained(
            MODEL_IMAGE)  # config for image
        config_text = AutoConfig.from_pretrained(
            args.model_name_or_path)  # config for language model - camembert

        model = LayoutLMvForSequenceClassification(
            config_image, config_text, args, MODEL_IMAGE)
